...
<script>
    ...
    var myLineChart = new Chart(ctx, {
        ...
        data: {
            ...
            ->where('created_at','>=',\Carbon\Carbon::now()->startOfHour()->subDay())->limit(24)->get()->map(function($check){
        return  $check->created_at->format('d.m.Y h\:00 a');
       })) !!},
            datasets:
            {!! json_encode(\App\Models\Provider::all()->map(function($provider){
           $data = $provider->checks()->where('check','=','server_creation_time')->where('created_at','>=',\Carbon\Carbon::now()->startOfHour()->subDay())->limit(24)->get()->map(function($check){
    return [
    'x' => $check->created_at->format('d.m.Y H:i:s'),
    'y' => (float) $check->result
    ];})->toArray();
    if(count($data) < \App\Models\Provider::find(1)->checks()->where('created_at','>=',\Carbon\Carbon::now()->startOfHour()->subDay())->where('check','=','server_creation_time')->limit(24)->count()){
    $diff = \App\Models\Provider::find(1)->checks()->where('created_at','>=',\Carbon\Carbon::now()->startOfHour()->subDay())->where('check','=','server_creation_time')->limit(24)->count() - count($data);
    ...
    var myLineChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels:{!! json_encode(\App\Models\Provider::find(1)->checks()->where('created_at','>',\Carbon\Carbon::now()->startOfHour()->subDay())->where('check','=','api_response_time')->limit(24)->get()->map(function($check){
        return  $check->created_at->format('d.m.Y h\:00 a');
       })) !!},
            datasets:
            {!! json_encode(\App\Models\Provider::all()->map(function($provider){
               $data = $provider->checks()->where('created_at','>',\Carbon\Carbon::now()->startOfHour()->subDay())->where('check','=','api_response_time')->limit(24)->get()->map(function($check){
    return [
    'x' => $check->created_at->format('d.m.Y H:i:s'),
    'y' => (float) $check->result
    ];})->toArray();
             if(count($data) < \App\Models\Provider::find(1)->checks()->where('created_at','>',\Carbon\Carbon::now()->startOfHour()->subDay())->where('check','=','api_response_time')->limit(24)->count()){
    $diff = \App\Models\Provider::find(1)->checks()->where('created_at','>',\Carbon\Carbon::now()->startOfHour()->subDay())->where('check','=','api_response_time')->limit(24)->count() - count($data);
